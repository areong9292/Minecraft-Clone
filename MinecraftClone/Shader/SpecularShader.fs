...
void main()
{
	...
	vec3 diffuse = diff * lightColor;							// 빛 세기 적용
	...
	float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);	// 반사 방향과 카메라 위치 내적, 하이라이트를 위한 pow 계산
	vec3 specular = specularStrength * spec * lightColor;		// 빛 세기 적용
